...
- [x] 실행 의존성과 dev, notebook, Studio 도구의 설치 경계를 분리한다.
...
- [x] 실패 가설·전략을 검색하는 `recall` node와 LangGraph Store 기반 장기
  기억을 추가한다.
...
## 완료된 구조 발견성과 실행 수명 정리

- [x] 주 agentic graph의 topology 정의와 production provider 조립을
  `builder.py`와 `composition.py`로 분리한다.
- [x] Agent Server·Studio·CLI·평가가 사용하는 graph와 진입점을 architecture
  문서의 graph inventory로 연결한다.
- [x] 요청 모델과 실제 LiteLLM client 모델을 하나의 effective model로
  해석하고 state·결과 provenance에 같은 값을 기록한다.
- [x] reset 시점에 이미 성공·데드락인 보드는 prompt·모델 호출 전에 종료한다.
- [x] 연구 실행은 agentic graph·provider·Store를 실험 범위에서 재사용한다.

완료 조건: 모델 선택, 초기 terminal route, Agent Server persistence 소유권과
두 graph family의 진입점을 테스트로 고정하고 전체 Python·viewer 검증을
통과한다.
