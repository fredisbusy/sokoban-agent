"""Build the reproducible structured-agent generalization notebook."""
...
_code = cast(Callable[[str], Any], new_code_cell)
_markdown = cast(Callable[[str], Any], new_markdown_cell)
_notebook = cast(Callable[..., Any], new_notebook)
_write = cast(Callable[[Any, Path], None], nbformat.write)


def build_notebook(output_path: Path) -> None:
    """Create an output-free notebook backed by the production runner."""
    ...
    notebook = _notebook(
        cells=[
            _markdown(
                ...
                "## 목적\n\n"
                "원시 행동 LLM, 구조화 정책, 국소 탐색, 설명 제거, 기존 "
                "full guard와 bounded A* oracle을 동일 held-out 사례에서 "
                "비교한다. 정책 실행은 제품과 같은 LangGraph runner를 쓰며 "
                "A*는 실행 후 비교 기준으로만 사용한다."
            ),
            _markdown(
                "## 재현성 계약\n\n"
            ...
            _code(
                ...
                "    OllamaSettings,\n"
                ")\n"
                "from sokoban_agent.planning.llm.planner import serialize_board\n\n"
            ...
            _code(
                ...
                "    record.to_flat_dict() for record in experiment.records\n"
                ")\n"
            ...
            _code(
                ...
                "}]\n"
                "for step, action_name in enumerate(\n"
                "    selected.outcome.action_sequence, 1\n"
                "):\n"
            ...
            _markdown(
                "## 해석 제한\n\n"
                "`current-full-guard`와 `astar-oracle`은 시스템 상한·사후 비교 "
                "기준이며 에이전트 독립 해결 성공률로 해석하지 않는다. "
                "bounded A* 실패는 해답 없음이 아니라 oracle 미해결이다."
            ),
        ],
        metadata={
            "kernelspec": {
    ...
    _write(notebook, output_path)


if __name__ == "__main__":
    build_notebook(Path("notebooks/agentic_generalization.ipynb"))
