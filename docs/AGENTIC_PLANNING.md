...
flowchart TD
    ...
    L --> C["compose_strategy_input · state를 prompt 변수로 조립"]
    C --> P["propose_strategy · 구조화된 가설과 하위 목표"]
    P --> V["verify · 규칙·제약·국소 실현성"]
    V -->|"거절·수정 가능"| P
    V -->|"유효"| X["execute · 제한된 행동/push"]
...
전략 Planner는 `BoardAnalysis`, 이전 계획 산출물, 실행 결과와 거절
피드백을 받아 다음 `StrategyHypothesis`와 하나의 `Subgoal`을 제안한다.
원시 행동은 직접 생성하지 않는다. 첫 Adapter는 구조화된 LLM 응답이고,
테스트와 ablation을 위한 규칙 기반 Adapter를 함께 둬 seam을 실제로 만든다.
...
- 실패 가설과 전략을 검색하는 장기 기억
- 화면 기반 perception과 인식 불확실성
- 더 강한 동적 데드락 분석
- 학습된 전략·가치 모델 또는 자동 curriculum

기억이나 perception을 먼저 추가해 계획 오류와 관찰 오류를 섞지 않는다.
