...
- `graph/`
  ...
  - `studio/`: 비교 baseline을 위한 레거시 Studio graph
- `evaluation/`
...
각 checkpoint에서 `BoardAnalysis`, 활성 가설, 현재 하위 목표, 보호 제약,
예상 효과, 검증 결과와 revision history를 확인할 수 있다.

`graph/studio/`의 원시 행동·Search Guard 그래프는 비교 baseline의 단계별
진단과 회귀 테스트를 위해 남아 있지만 `langgraph.json`의 Agent Server
진입점은 아니다.
