...
현재 주 실행 경로는 보드 분석부터 전략 제안·검증, 단일 push 실행, 성찰과
memory를 반복하는 구조화 `StateGraph`다. 원시 행동 Planner와 전역 탐색
guard 그래프는 비교 baseline으로 유지한다. 두 실행 경로 모두 별도 자체
workflow를 만들지 않고 LangGraph의 `StateGraph`, state reducer,
conditional edge, retry policy와 checkpointer를 사용한다.
...
`langgraph.json`은 주 그래프 하나를 가리킨다. Agent Server·Studio는
composition root가 만든 전역 compiled graph를 사용하고, CLI·평가는 같은
factory와 의존성 조립 규칙으로 local graph를 한 번 compile해 재사용한다.
JSON-safe 표시 변환이 필요하면 state 또는 노드의 얇은 Adapter로 제한한다.

## 그래프 인벤토리

NestJS의 module graph처럼 “어떤 진입점이 어떤 graph와 provider를 조립하는가”를
다음 표에서 먼저 찾는다. A* oracle은 평가용 solver이지 LangGraph가 아니므로
그래프 수에 포함하지 않는다.

| 그래프 | 정의·상태 | 조립·진입점 | 주 소비자 |
| --- | --- | --- | --- |
| 구조화 agentic | `graph/agentic/builder.py`, `AgenticState` v2 | `graph/agentic/composition.py`; Agent Server는 `langgraph.json`, local은 `AgenticGraphRunner` | Agent Server·Studio·CLI·agentic 평가 |
| 원시 행동 baseline | `graph/baseline/`, `SokobanGraphState` v2 | `SokobanGraph`에 `Planner` 주입 | Random·BFS·LLM·Search Guard 비교 평가 |

`builder.py`는 node와 edge만 연결한다. 운영 provider 선택과 수명은
`composition.py`, local checkpoint·Store와 thread 실행은
`AgenticGraphRunner`, 정책별 결과 투영은 `evaluation/`이 맡는다.
따라서 새 모듈의 위치는 “어느 그래프인가”보다 먼저 “도메인 계산인가,
그래프 orchestration인가, 외부 adapter인가”로 결정한다.

## 계획 상태와 도메인 노드

주 그래프의 중심은 원시 행동열이 아니라 구조화된 계획 상태다.
...
- 개발·운영 승격이 필요할 때 LangSmith의 `staging`·`production` tag를
  사용한다.
- prompt 이름·commit과 요청 모델은 runtime context로 주입한다. strategy
  provider가 실제 사용할 모델을 해석한 뒤 같은 effective model을 state와
  결과 manifest에 보존한다.
...
prompt provenance 값 객체를 조합한다. `AgenticState`도 실행 필드와 계측을
섞지 않고 `metrics.strategy/llm/memory/local_search/rules`의 JSON-safe
채널로 보존한다. 반면 연구 JSON과 pandas는 분석용 평면 행이므로
`ResearchEpisodeRecord.to_flat_dict()` Adapter에서만 컬럼을 평탄화하고
`record_schema_version`으로 계약 변경을 표시한다. v2 artifact는 정책에
적용되지 않는 metric group을 0이 아닌 `null`로 내보내 측정값 0과
N/A를 구분한다.

`AgenticState.metrics` 도입은 checkpoint schema 변경이다. 이전 scalar
metric checkpoint는 새 graph에서 resume하지 않으며, 새 run과 thread ID를
사용한다.
...
`composition.py`가 prompt source와 strategy generator를 조립한다.
Agent Server·Studio는 `langgraph.json`의 전역 compiled graph를 사용하고,
CLI와 평가는 `AgenticGraphRunner`가 같은 factory를 local `InMemorySaver`와
함께 호출한다. 연구 실행은 runner 하나를 전체 level·seed grid에서 재사용해
graph와 provider를 에피소드마다 다시 만들지 않는다. 실행기는 행동·재계획
루프를 구현하지 않으며 최종 graph state의 계측만 결과 계약으로 투영한다.
원시 행동 baseline의 기존 graph는 비교 정책을 재현하기 위해서만 유지한다.
...
`action_count`와 하위 목표 성공·실패처럼 canonical sequence/effect에서
계산할 수 있는 값은 내부 저장 필드로 복제하지 않고 property와 artifact
Adapter에서 파생한다.
...
`langgraph.json`은 `graph/agentic/composition.py`의 주 구조화 그래프를
가리킨다. Studio와 Agent Server는 별도 workflow 없이 `analyze`,
`resolve_prompt`, `propose_strategy`, `verify_strategy`, memory recall,
`ground_subgoal`, `execute_until_push`, `reflect`, `observe` 전이를 표시한다.
각 checkpoint에서 `BoardAnalysis`, 활성 가설, 가설에서 파생되는 현재 하위
목표·보호 제약·예상 효과, 검증 결과와 revision history를 확인할 수 있다.
원시 행동 정책 비교는 별도 Studio graph가 아니라 `graph/baseline/` 평가
runtime과 versioned artifact adapter를 사용한다.

Agentic checkpoint schema는 현재 v2다. v1 또는 version이 없는 기존 thread는
자동 변환하지 않으며 명시적인 schema mismatch로 중단한 뒤 새 thread에서
실행한다. 반면 baseline episode·summary JSON은 v1 flat adapter를 통해 기존
컬럼을 유지하고, 내부 dataclass 중첩은 외부 artifact 계약에 노출하지 않는다.

## 다음 확장

LangGraph Store 기반 분석·전략·접지 memory까지 구현되어 있다. 다음 확장은
화면 기반 perception, 더 강한 freeze/tunnel deadlock과 학습된 전략
모델이며, 각 기능은 기존 계획 성능과 독립적으로 측정한 뒤 추가한다.
