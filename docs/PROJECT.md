...
LiteLLM 기반 Ollama 클라이언트와 한 행동씩 다시 계획하는 LLM Agent도
구현됐다. 보드 직렬화, JSON Schema 행동 출력, 응답·막힌 행동 재시도와
LLM 진단 지표를 지원한다. 비교 노트북에는 내장 2개와 실험용 미지 레벨
1개에서 Random/BFS/LLM을 같은 설정으로 실행한 결과가 기록됐다. 집계에
사용된 각 에피소드의 실제 상태와 행동도 trajectory로 남겨 노트북에서
애니메이션으로 재생할 수 있다.
