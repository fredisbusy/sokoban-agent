...
각각 실행한 뒤 <http://127.0.0.1:4173>에서 사용할 수 있다. 화면은 Next.js
...
실제 모델을 사용한 `tiny-walk` run에서도 첫 행동이 run 종료 전에
표시됐다. 최종 화면의 board, 5개 행동, 2번의 push, 전략과 예상·관찰 효과가
Agent Server 최종 state와 일치했고 브라우저 console 오류가 없었다.

## 범위

첫 버전은 다음 기능을 제공한다.

- 개발용 맵과 Boxoban 공식 난이도별 연구 맵, seed와 행동 제한 입력
- LangSmith prompt의 최신 commit 자동 해석, 모델과 ablation runtime
  context 입력
...
주 그래프의 `execute_until_push`가 반환한 `board`, `observation`, `action`과
상태를 그대로 표시한다.
...
`prompt_name`, `model_name`, `rationale_mode`, `grounding_mode`를 실행
폼에서 받고 `prompt_commit`에는 `latest`를 자동으로 넣어 Agent Server의
공식 per-run `context`로 전달한다. LangGraph의 `resolve_prompt` node가 이를
실행 시점의 실제 immutable commit으로 해석하고 state에 기록한다. 엄격한
재현성이 필요한 연구 배치에서는 기존처럼 고정 commit을 직접 주입한다.

맵 선택기는 Boxoban 연구 맵과 `levels/custom/*.json`을 합쳐 생성한
`src/sokoban_agent/data/level_catalog.json`을 단일 실행 기준으로 사용한다.
브라우저는 선택한 `level_id`와 `level_sha256`을 graph input으로 보내고,
Agent Server는 같은 catalog에서 실제 `level_rows`를 해석한다. 임시 fixture와
테스트는 기존 `level_rows` 직접 입력 계약을 사용할 수 있다.
...
6. CLI·평가와 같은 graph 정의를 사용하는 Agent Server run을 표시한다.
