...
맵 선택기는 `benchmarks/boxoban_research_v1.json`을 단일 기준으로 사용한다.
브라우저는 선택한 `level_id`와 `level_rows`를 graph input으로 보낼 뿐
Sokoban 규칙을 계산하지 않는다. 따라서 외부 원본 경로를 Agent Server에
직접 노출하거나 별도 level API를 만들지 않으며, `initialize` node가 기존
`level_rows` 계약으로 보드를 검증·초기화한다.
