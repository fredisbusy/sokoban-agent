...
## 2026-07-24 현재 상태와 추가 단계

- P0–P2: 외부 평가 row 계약과 typed payload는 구현 완료 상태를 유지한다.
- P3–P5: legacy Studio 제거, viewer decoder, nested evaluation/state 계약과
  checkpoint schema guard가 구현되어 있다.
- P6: 문서는 대부분 갱신되었으나 graph 진입점 발견성과 provider 수명 계약이
  부족했다.

### P7. Composition과 provenance

- `builder.py`는 node/edge topology만 소유한다.
- `composition.py`가 prompt source와 strategy generator를 조립한다.
- strategy generator가 요청 model을 실제 client 설정으로 해석하고 그 값을
  state·memory key·결과 provenance에 공통으로 사용한다.
- local runner는 checkpointer·Store를, Agent Server는 플랫폼 persistence를
  소유한다.
- 연구 runner는 하나의 compiled graph와 provider를 전체 실험에서 재사용한다.
- graph inventory는 구조화 agentic과 baseline 두 family의 진입점·상태·소비자를
  명시한다.

완료 조건: terminal reset에서 모델 호출이 0회이고, 요청 model로 생성한 client가
재사용되며, Agent Server graph의 local Store가 비어 있고 전체 품질 게이트가
통과한다.
